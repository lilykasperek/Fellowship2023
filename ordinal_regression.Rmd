...
```{r}
library(readr)
modeling_data <- read_csv("data/modeling_data_student_by_student.csv")
modeling_data
```
...
newdat <- cbind(mod_df_filter, predict(both_mod_2, type = "probs")) |>
  dplyr::select(-reason, -year, -gender, -Response, -Semester, -Course,
                -response, -new_responses)
lnewdat <- melt(newdat, id.vars = c("Reason", "Year", "Gender"),
                variable.name = "Level", value.name = "Probability")
head(lnewdat)
ggplot(lnewdat, aes(x = Reason, y = Probability, colour = Level)) +
  geom_point() + 
  geom_line(group = 1) +
  facet_grid(Year ~ Gender, labeller="label_both")

ggplot(lnewdat, aes(x = Probability, colour = Level)) +
  geom_freqpoly() +
  facet_grid(Year ~ Gender, labeller="label_both")
```
